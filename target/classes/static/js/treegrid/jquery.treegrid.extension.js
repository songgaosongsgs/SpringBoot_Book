        ...
        target.getChildNodes = function(data, parentNode, parentIndex, tbody) {
            $.each(data, function(i, item) {
                if (item[options.parentCode] == parentNode[options.code]) {
                    ...
                    if(item.checked) tr.addClass('treegrid-selected');
        ...
        target.renderRow = function(tr,item){
            $.each(options.columns, function(index, column) {
                ...
                if(index==0&&column.field=='selectItem'){
                    ...
                    if(column.radio){
                        var _ipt = $('<input name="select_item" type="radio" value="'+item[options.id]+'" ' + (item.checked?'checked':'') + ' ></input>');
                        td.append(_ipt);
                    }
                    if(column.checkbox){
                        var _ipt = $('<input name="select_item" type="checkbox" value="'+item[options.id]+'" ' + (item.checked?'checked':'') + ' ></input>');
                        td.append(_ipt);
                    }
                    tr.append(td);
                }else{
                    ...
                    if (column.formatter) {
                        ...
                        td.text(item[column.field]?item[column.field]:'');
                    }
                    tr.append(td);
                }
            });
        ...
        target.load = function(parms){
            ...
            $.each(options.columns, function(i, item) {
                ...
                if(i==0&&item.field=='selectItem'){
                    hasSelectItem = true;
                    if(item.checkbox){
                    	th = $('<th style="width:36px"><input name="select_item_all" type="checkbox" /></th>');
                    }else{
            ...
            $.ajax({
                ...
                success : function(data, textStatus, jqXHR) {
                	if(data.code && data.code == 200 && data.data) data = data.data; 
                    ...
                    // 行点击选中事件
                    target.find("tbody").find("tr").click(function(e){
                    	if(e.target.nodeName == 'span' || e.target.nodeName == 'SPAN') return;
                    ...
                    target.find("thead").find('input:checkbox[name="select_item_all"]').click(function(){
                    	target.find("tbody").find('input:checkbox[name="select_item"]').prop('checked', this.checked);
                	}) 
                },
